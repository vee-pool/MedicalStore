...
public class ExpiryDateReport extends JFrame implements ActionListener
 {
    JFrame jf;
  ...
  public ExpiryDateReport()
  {
   ...
   jf.setLayout(null);
  	...
  	ln = new JLabel("Medicine Expiry report");
  ...
  l1.setBounds(50,100,250,25);
    ...
    t1.setBounds(300,100,100,25);t1.setToolTipText("Enter date");
    jf.add(t1);
    ...
    jf.setTitle("Expiry Date Report");
    jf.setSize(900,700);
	...
	if(ae.getSource()==submit)
	 {//list
	        String expdt=t1.getText();
		Pattern p1=Pattern.compile("[0-9]+[-][0-9]+[-][0-9]+");
		Matcher m1=p1.matcher(expdt);
	...
	else if(!matchFound1)
  {
        ...
        rs = stmt.executeQuery("SELECT sid,sname,mbno,mname,mexpdate,mqty,mpurprice,msaleprice from medicine where mexpdate<='"+t1.getText()+"' ");
          while(rs.next())
            {
              ...
              if (foundrec == 0)
                {
                    JOptionPane.showMessageDialog(null,"Not any medicine Expired till this date","Dialog",JOptionPane.WARNING_MESSAGE);
                }
	        }
             con.close();
       }
